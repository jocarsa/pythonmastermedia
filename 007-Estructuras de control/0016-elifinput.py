print("Introduce el día de la semana")
diadelasemana = input()

if diadelasemana == "lunes":
    print("hoy es el peor día de la semana")
elif diadelasemana == "martes":
    print("hoy es el segundo peor día de la semana")
elif diadelasemana == "miércoles":
    print("ya estamos a mitad de semana")
elif diadelasemana == "jueves":
    print("ya es juernes")
elif diadelasemana == "viernes":
    print("ya se acaba la semana")
elif diadelasemana == "sábado":
    print("hoy es el mejor día de la semana")
elif diadelasemana == "domingo":
    print("parece mentira que mañana ya sea lunes")
else:
    print("lo que has escrito no es un día de la semana")



